...
import { useGetPendingCompanyReviewsQuery, useAcceptPendingCompanyReviewMutation, useDeclinePendingCompanyReviewMutation } from "@/app/state/api"
...
const CompanyReviewPendingTab = () => {
  const [currentPage, setCurrentPage] = useState(1)
  const [processingIds] = useState<string[]>([])
  const { data: pendingReviews = [], refetch } = useGetPendingCompanyReviewsQuery()
  const [acceptReview, {isLoading: isApproving}] = useAcceptPendingCompanyReviewMutation()
  const [declineReview, {isLoading: isDeclining}] = useDeclinePendingCompanyReviewMutation()
              ...
              <TableBody>
                {currentReviews.map((review) => (
                  <TableRow key={review.reviewId}>
                    ...
                    <TableCell className="align-top">
                      <div className="max-h-36 w-64 overflow-y-auto text-sm p-3 whitespace-normal">
                        <p className="text-xs text-muted-foreground">{review.Review.Company.companyName}</p>
                        <span className="text-xs text-muted-foreground">{review.Review.Company.fullAddress}</span>
                      </div>
                    </TableCell>
                    <TableCell className="align-top">
...
export default CompanyReviewPendingTab
